 ...
 * state for which general components should be displayed by the app. At most, 
 * actions.displays should have exactly one of it's register, login, and home
 * properties set to true. If an account is logged into the app (i.e. there is
 * a jwToken in localStoreage), then home component will be set to true while 
 * register and login components set to false, even if home was not attempted 
 * to be set to true in actions.displays. If more or less than one of these
 * components is true in actions.displays, then home will take priority (if an 
 * account is logged in), followed by login (if no account is logged in). If any
 * expected properties in actions.displays (e.g. resgister, login, ect.) are
...
export default function generalComponentsDisplayReducer(
	...
	switch (action.type) {
		case SET_WHICH_GENERAL_COMPONENTS_DISPLAY:
			...
			const keysOfRegisterLoginHomeComponentsComponentsSetToTrue = Object.keys(
			...
			if (localStorage.getItem("jwToken") !== null) {
				if (action.displays.home !== true) {
					...
					keysOfRegisterLoginHomeComponentsComponentsSetToTrue.length > 1
				) {
					console.log(
						"Error: " +
							getStringOfAllArrayValues(keysOfRegisterLoginHomeComponentsComponentsSetToTrue) +
			...
			else if (keysOfRegisterLoginHomeComponentsComponentsSetToTrue.length > 1)
			{
