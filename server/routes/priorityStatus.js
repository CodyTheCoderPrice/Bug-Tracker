...
router.route("/retrieve").get( async (req, res) => {
	...
	try {
		const projectPriority = await pool.query(
			"SELECT p_priority_id AS id, option FROM project_priority ORDER BY order_number"
		);

		const projectStatus = await pool.query(
			"SELECT p_status_id AS id, option FROM project_status ORDER BY order_number"
		);

		const projectStatusCompletionIndex = await pool.query(
			"SELECT p_status_id AS id FROM project_status WHERE marks_completion = true"
		);

		const bugPriority = await pool.query(
			"SELECT b_priority_id AS id, option FROM bug_priority ORDER BY order_number"
		);

		const bugStatus = await pool.query(
			"SELECT b_status_id AS id, option FROM bug_status ORDER BY order_number"
		);

		const bugStatusCompletionIndex = await pool.query(
			"SELECT b_status_id AS id, option FROM bug_status WHERE marks_completion = true"
		);
